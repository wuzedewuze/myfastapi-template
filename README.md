...
- 🎯 Entry points to execute your program using `python -m <myfastapi_template>` or `$ myfastapi_template` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/wuzedewuze/myfastapi-template/branch/main/graph/badge.svg?token=myfastapi-template_token_here)](https://codecov.io/gh/wuzedewuze/myfastapi-template)
[![CI](https://github.com/wuzedewuze/myfastapi-template/actions/workflows/main.yml/badge.svg)](https://github.com/wuzedewuze/myfastapi-template/actions/workflows/main.yml)

Awesome myfastapi_template created by wuzedewuze

## Install

from source
```bash
git clone https://github.com/wuzedewuze/myfastapi-template myfastapi_template
cd myfastapi_template
make install
```
...
```bash
pip install myfastapi_template
```

## Executing

```bash
$ myfastapi_template run --port 8080
```

or

```bash
python -m myfastapi_template run --port 8080
```

or

```bash
$ uvicorn myfastapi_template:app
```

## CLI

```bash
❯ myfastapi_template --help
Usage: myfastapi_template [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ myfastapi_template create-user --help
Usage: myfastapi_template create-user [OPTIONS] USERNAME PASSWORD
...
```bash
myfastapi_template create-user admin admin --superuser
```
...
Run with `myfastapi_template run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `myfastapi_template create-user`.
...
-----------------------------------------------------
myfastapi_template/__init__.py              4      0   100%
myfastapi_template/app.py                  16      1    94%
myfastapi_template/cli.py                  21      0   100%
myfastapi_template/config.py                5      0   100%
myfastapi_template/db.py                   10      0   100%
myfastapi_template/models/__init__.py       0      0   100%
myfastapi_template/models/content.py       47      1    98%
myfastapi_template/routes/__init__.py      11      0   100%
myfastapi_template/routes/content.py       52     25    52%
myfastapi_template/routes/security.py      15      1    93%
myfastapi_template/routes/user.py          52     26    50%
myfastapi_template/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in myfastapi_template/default.toml.
